...
def atualizar_treeview(comando, data_frame):
    ...
    label_comando = tk.Label(frame_interacao, text=f"Comando: {comando}", font=("Arial", 12, "bold"), bg="#f0f0f0", fg="#333")
    ...
    btn_exportar = tk.Button(frame_interacao, text="Exportar para Excel", command=lambda: exportar_para_excel(data_frame, comando), bg="#4CAF50", fg="white", font=("Arial", 10))
...
def processar_comando():
    ...
    if comando.startswith("delete a coluna"):
        ...
        if len(partes) == 2:
            ...
            if coluna.lower() in colunas_lower:
                ...
                if not df_filtrado.empty:
                    ...
                    messagebox.showwarning("Atenção", f"Nenhum dados encontrado para '{valor}' na coluna '{coluna_real}'.")

            else: messagebox.showerror("Erro", f"A coluna '{coluna}' não existe no DataFrame.")
        ...
        try:
            ...
            math = re.match(padrao, comando)

            if math:
                coluna_grupo = math.group(1).strip()
                mais_ou_menos = math.group(2).strip()
                coluna_vendas = math.group(3).strip()
                ...
                if coluna_grupo_key in colunas_lower and coluna_vendas_key in colunas_lower:
                    ...
                    if mais_ou_menos == 'mais':
                        grupo_selecionado = grupo_vendas.idmax()
        ...
        try:
            ...
            if match:
                ...
                if coluna_grupo_real and coluna_vendas_real:
                    ...
                    grupo_vendas.sort_values(by=coluna_grupo_real, ascending=False, inplace=True)
...
def mostrar_dicas():
    ...
    text_dicas = """Exemplos de Comandos:
...
"""
    ...
    texto = tk.Text(janela_dicas, wrap='word', height=20)
    texto.insert('1.0', text_dicas)
    texto.pack(expand=True, fill='both')

    def copiar_dicas():
        janela_principal.clipboard_clear()
        janela_principal.clipboard_append(text_dicas)

        messagebox.showinfo("Copiado", "Dicas copiadas para a área de transferência!")

    btn_copiar = tk.Button(janela_dicas, text="Copiar Dicas", command=copiar_dicas, bg="#4CAF50", fg="white", font=("Arial", 10))
...
scrollbar_chat = ttk.Scrollbar(frame_principal, orient="vertical", command=canvas.yview)
...
frame_inferior.pack(side="bottom", fill="x", padx=10, pady=5)

entry_comando = tk.Entry(frame_inferior, width=60, font=("Arial", 12), fg="#333")
entry_comando.pack(side="left", padx=5)

btn_comando = tk.Button(frame_inferior, text="Executar", font=("Arial", 12), command=processar_comando, fg="#FFFFFF", bg="#38906A" )
btn_comando.pack(side="left", padx=5)

btn_carregar = tk.Button(frame_inferior, text="Carregar Arquivo Excel", command=carregar_arquivo, font=("Arial", 12), bg="#2196F3", fg="white")
btn_carregar.pack(side="left", padx=5)

btn_dicas = tk.Button(frame_inferior, text="Dicas", command=mostrar_dicas, font=("Arial", 12), bg="#FF5722", fg="white")
